...
.controller('all', ["$scope", "$timeout", "$mdSidenav", "$mdDialog", "$window", "$http", function all($scope, $timeout, $mdSidenav, $mdDialog, $window, $http) {
			...
			findid = function () {
				if (location.pathname.indexOf("collection") !== -1) {
					$scope.user.id = geniussource.responseJSON.response.user.id;
				} else {
					$scope.user.id = JSON.parse(source.responseJSON.query.results["preload-content"]["data-preload_data"]).user.id;
				}
			};

			/* findavatar = function () {
				$scope.user.avatar = geniussource.responseJSON.response.user.avatar;
			};

			findname = function () {
				$scope.user.name = geniussource.responseJSON.response.user.name;
			};

			findlogin = function () {
				$scope.user.login = geniussource.responseJSON.response.user.login;
			};
			...
			findiq_for_display = function () {
				$scope.user.iq_for_display = geniussource.responseJSON.response.user.iq_for_display;
			};

			findrole_for_display = function () {
				$scope.user.role_for_display = geniussource.responseJSON.response.user.role_for_display;
			};

			findroles_for_display = function () {
				$scope.user.roles_for_display = geniussource.responseJSON.response.user.roles_for_display;
			};

			findiq = function () {
				$scope.user.iq = geniussource.responseJSON.response.user.iq;
			};

			findtranscriptions_count = function () {
				$scope.user.transcriptions_count = geniussource.responseJSON.response.user.stats.transcriptions_count;
			};

			findannotations_count = function () {
				$scope.user.annotations_count = geniussource.responseJSON.response.user.stats.annotations_count;
			}; */

			$scope.find = function () {
				$scope.$apply(function () {
					$scope.user = geniussource.responseJSON.response.user;
				});
			};
			...
			insert = function () {
				$scope.insertbadges();
				$scope.styles.starscontainer.height = $scope.round(((8 * $scope.user.stars / 51 + 17 / 5) + 2), 3) + "vh";
				$scope.styles.stars["font-size"] = $scope.round((8 * $scope.user.stars / 51 + 17 / 5), 3) + "vh";
				/*
				insertavatar();
				insertstats();
				insertnamelink();
			...
			$scope.doit = function () {
				...
				$.get(collectionspath + $scope.user.login.toUpperCase().replace(/\s+/g, "-") + ".js")
				.done(function (data) {
					...
					if (location.pathname.indexOf("collection") == -1) {
						...
						window.history.pushState({}, document.title, currentpath.slice(0, (nthIndex(currentpath, "/", 3) + 1)) + $scope.user.login.toUpperCase().replace(/\s+/g, "-").toLowerCase());
